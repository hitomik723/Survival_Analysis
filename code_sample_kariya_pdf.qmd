...
adrd_act <-
  adrd_act %>% select("studyid","bln_flag_dead_asof_30sep2016",
                      "bln_onsetdate", "bln_deathdt", "bln_withdrawdt",
                      "dql_true_elder_orphan", "bln_age_at_onset",
                      "bln_education", "bln_gender", "ein_charlson",
                      "bln_race", "bln_hispanic")

# change column names 
adrd_act <- 
  setnames(adrd_act,
           old = c("studyid", "bln_flag_dead_asof_30sep2016", "bln_onsetdate",
                   "bln_deathdt", "bln_withdrawdt", "dql_true_elder_orphan",
                   "bln_age_at_onset", "bln_education", "bln_gender",
                   "ein_charlson", "bln_race", "bln_hispanic"),
           new = c("studyid", "flag_death", "onset_date", "death_date",
                   "withdraw_date", "elder_orphan", "age_onset", "education",
                   "sex", "charlson", "race", "ethnicity"))
```
...
# 1 = male, 2 = female -> 0 = female, 1 = male (female as ref)
...
adrd_act$surv_years <- 
  ifelse((adrd_act$flag_death == 0 & is.na(adrd_act$withdraw_date)) |
           (adrd_act$flag_death == 0 & adrd_act$withdraw_date > "2016-09-30"),
...
(table1 <- table1(~ surv_years + age_onset + race_bin + ethnicity_bin + sex +
                    educ_bin + charlson | elder_orphan, data = adrd_act))
```
...
theme(plot.title = element_text(size = 5, color = "black"),
...
orphan_ph <- coxph(tte ~ elder_orphan + age_onset + educ_bin + sex + charlson,
                   data = adrd_act)
...
sa_rope$signif <- ifelse(sa_rope$signif == TRUE,
                         "Significant",
                         "Insignificant")
